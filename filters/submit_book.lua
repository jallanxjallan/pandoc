...
function BulletList(items)
    for _, item in pairs(items) do
        -- Prepend "Feature" header to the list item content
        table.insert(collected_texts, "# Feature\n")
        
        -- Process the list item
        for _, elem in pairs(item) do
            if elem.t == "Para" then
                Para(elem)  -- Reuse the Para function to handle inline links in list items
            end
        end
    end
end
